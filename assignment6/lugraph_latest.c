...
{
  if (from < 0 || from >= g->n) return false;

	lug_search *result = lug_search_create(g, from);
  
  //
  //printf("created lug_search struct result\n");
  //
  ...
  //push from onto queue
  result->pred[from] = from; // set previous node of from to itself
  result->color[from] = GRAY;
  result->dist[from] = 0;
  q[tail++] = from;

  while(head < tail)
  {
    current = q[head++]; // dequeue
    //printf("%d, distance = %d\n", current, result->dist[current]);
    //printf("  nbr: ");
    for (int i = 0; i < g->list_size[current]; i++) //index through all neighbors of current
    {
      nbr = g->adj[current][i];
      //printf("(%d, %d) ", nbr, result->color[nbr]);
      if (result->color[nbr] == WHITE)
      {
        result->color[nbr] = GRAY;
        result->dist[nbr] = result->dist[current] + 1;
        result->pred[nbr] = current;
        q[tail++] = nbr;
      }
    }
    //printf("\n");
    result->color[current] = BLACK;
  }
...
{
  if (from < 0 || from >= g->n || to < 0 || to >= g->n) return false;

  lug_search *search = lugraph_bfs(g, from);

  if (search->color[to] == BLACK)
  {
...
{
  if (to < 0 || to >= s->g->n) return -1;
  if (s->color[to] == WHITE) return -1;
  return s->dist[to];
}
